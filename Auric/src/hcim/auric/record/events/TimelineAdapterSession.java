...
public class TimelineAdapterSession extends BaseAdapter {
	...
	public TimelineAdapterSession(Context context, List<Intrusion> intrusions) {
		this.context = context;
		this.logDB = EventBasedLogDatabase.getInstance(context);
		...
		for (Intrusion i : intrusions) {
			...
			if (log.isListEmpty()) {
				items.add(new EventBasedLogItem(i.getTime(), intruders, color, i.getID()));
			} else {
	...
	private int getColor(Intrusion i) {
		if (i.getTag() == Interaction.FALSE_INTRUSION) {
			return R.color.lightgreen;
		}
		return R.color.lightred;
	}

	private void organize(List<Picture> intruders, EventBasedLog log, int color, String intrusionID) {
...
//	}
	...
	private View getView(Bitmap b) {
		...
		//img.setImageBitmap(b); //TODO
		img.setImageResource(R.drawable.blur);
		return view;
	}
