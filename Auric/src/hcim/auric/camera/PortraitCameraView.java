...
public class PortraitCameraView extends CameraBridgeViewBase implements
		PreviewCallback {
			...
			try {
				...
				if (sizes != null) {
					...
					Size frameSize = calculateCameraFrameSize(sizes,
							new JavaCameraSizeAccessor(), height, width); // use
																			// turn
																			// around
																			// values
																			// here
					...
					mFrameWidth = params.getPreviewSize().height; // the frame
																	// width and
																	// height of
																	// the super
																	// class are
					...
					int realWidth = mFrameHeight; // the real width and height
					...
					mFrameChain[0] = new Mat(realHeight + (realHeight / 2),
							realWidth, CvType.CV_8UC1); // the frame chane is
														// still in landscape
					mFrameChain[1] = new Mat(realHeight + (realHeight / 2),
							realWidth, CvType.CV_8UC1);

					AllocateCache();

					mCameraFrame = new JavaCameraFrame[2];
					mCameraFrame[0] = new JavaCameraFrame(mFrameChain[0],
							mFrameWidth, mFrameHeight); // the camera frame is
														// in portrait
	...
	private class JavaCameraFrame implements CvCameraViewFrame {
		...
		public Mat gray() {
			if (mRotated != null)
				mRotated.release();
			mRotated = mYuvFrameData.submat(0, mWidth, 0, mHeight); // submat
																	// with
																	// reversed
																	// width and
																	// height
